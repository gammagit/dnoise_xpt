function out_pars = init_params()
    ...
    out_pars.ntrials = 5; % number of trials in block (if not fixed-time)
    ...
    out_pars.con.const = [0.70 0.70 0.70]; % contrast in variable noise xpt
    ...
    out_pars.con.calib = 1.2; % contrast if noise is being calibrated
    ...
    out_pars.nwup = [1 1]; %[10 5]; % warm-up trials in each calibration block
