...
parameters {
    ...
    //real<lower=0.00001,upper=1> tau[NS];
    //real<lower=0.001,upper=1> lambda[NS];
    
    real <lower=0> group_mu_mu;
    ...
    real <lower=0.001,upper=100> group_lambda_dev;
}
...
model {
    ...
    group_mu_mu ~ normal(800, 500); // Large standard-deviation
    group_mu_precision ~ gamma(0.00001, 0.1); // Prior high on low precision, but heavy tail
    group_precision_mean ~ gamma(0.00001, 0.05); // Mean sigma ~ 70, large weight on low precision
    group_precision_dev ~ gamma(0.1, 0.1); //
    group_lambda_mean ~ gamma(0.0005, 0.05); // 
    group_lambda_dev ~ gamma(0.1,0.1); //
}
